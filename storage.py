...
class SecureStorage:
    ...
    def _get_master_password(self, gui_password=None):
        """Get or set master password for encryption"""
        if self._master_password is not None:
            return self._master_password

        # If GUI provided password, use it
        if gui_password is not None:
            self._master_password = gui_password
            return self._master_password

        # Check if this is first time setup
        if not self.config_file.exists():
            print(
                "First time setup - Create a master password to secure your accounts"
            )
    ...
    def _get_fernet(self, gui_password=None):
        """Get Fernet encryption instance"""
        if self._fernet is not None:
            return self._fernet

        try:
            password = self._get_master_password(gui_password)
            salt = self._get_salt()
            ...
            if self.config_file.exists():
                self._load_accounts(
                )  # This will raise exception if password is wrong
    ...
    def _load_accounts(self, gui_password=None):
        ...
        try:
            ...
            fernet = self._get_fernet(gui_password)
            ...
            if "Invalid token" in str(e):
                raise ValueError(
                    "Cannot decrypt accounts - incorrect password or corrupted data"
                )
            raise

    def _save_accounts(self, accounts, gui_password=None):
    ...
    def add_account(self, name, secret, gui_password=None):
        """Add a new account with encrypted secret"""
        try:
            accounts = self._load_accounts(gui_password)
            ...
            return self._save_accounts(accounts, gui_password)

        except Exception as e:
            print(f"Error adding account: {str(e)}")
            return False

    def remove_account(self, name, gui_password=None):
        """Remove an account"""
        try:
            accounts = self._load_accounts(gui_password)
    ...
    def get_secret(self, name, gui_password=None):
    ...
    def list_accounts(self, gui_password=None):
    ...
    def account_exists(self, name, gui_password=None):
